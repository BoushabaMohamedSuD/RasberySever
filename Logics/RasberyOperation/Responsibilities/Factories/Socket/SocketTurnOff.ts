...
export class SocketTurnOff implements RasberyResponsabilities {
    ...
    public processOperation(): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            this.process().subscribe(
                (resp) => {
                    if (resp) {
                        console.log('Socket turn off an response');
                        resolve(true);
                    }
                },
                (err) => {
                    console.log('Error in socket turn off');
                    reject(false);

                },
                () => {
                    console.log('socket turn off complete');
                }
            )
        });
