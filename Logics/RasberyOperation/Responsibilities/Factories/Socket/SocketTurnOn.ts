...
export class SocketTurnOn implements RasberyResponsabilities {
    ...
    public processOperation(): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            this.process().subscribe(
                (resp) => {
                    if (resp) {
                        console.log('Socket turn on an response');
                        resolve(true);
                    }
                },
                (err) => {
                    console.log('Error in socket turn on');
                    reject(false);

                },
                () => {
                    console.log('socket turn on complete');
                }
            )
        });
