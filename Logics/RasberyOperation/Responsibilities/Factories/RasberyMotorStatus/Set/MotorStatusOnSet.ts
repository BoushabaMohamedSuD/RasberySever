...
export class MotorStatusSetOn implements RasberyResponsabilities {
    ...
    public processOperation(): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            this.process().subscribe(
                (resp) => {
                    if (resp) {
                        console.log('Motor set status on  achieved an response');
                        resolve(true);
                    }
                },
                (err) => {
                    console.log('Error in Motor set status on');
                    reject(false);

                },
                () => {
                    console.log('Motor set status on  complete');
                }
            )
        });
    ...
    public process(): Observable<boolean> {
        return new Observable((observer: Observer<boolean>) => {
            RasberySql.update({ isActive: true }, { where: { id: 1 } })
                .then(() => {
