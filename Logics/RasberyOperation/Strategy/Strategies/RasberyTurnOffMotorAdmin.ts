...
import { TurnOffMotor } from './../../Responsibilities/res/TurnOffMotor';
...
export class RasberyTurnOffMotorAdmin implements RasberyStrategy {
    ...
    constructor(request: Request<ParamsDictionary, any, any>, response: Response<any>) {
        ...
        const chaine4 = FactoryMotorStatus.OpStatus(request, response, this.data, 'on', 'check');
        const chaine5 = FactorySocket.OpRuntime(request, response, this.data, 'wait');
        const chaine6 = new TurnOffMotor(request, response, this.data);
        const chaine7 = FactoryMotorStatus.OpStatus(request, response, this.data, 'off', 'set');
        const chaine8 = new AddNotification(request, response, this.data);
        const chaine9 = FactorySocket.OpRuntime(request, response, this.data, 'turnoff');
