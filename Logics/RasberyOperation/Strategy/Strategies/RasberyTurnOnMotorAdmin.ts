...
export class RasberyTurnOnMotorAdmin implements RasberyStrategy {
    ...
    public processOperation(): Promise<boolean> {
        console.log("Rasbery trun on motor admin startegy");
        return new Promise((resolve, reject) => {

            this.chaine1.processOperation()
                .then((resp) => {
                    if (resp) {
                        console.log('succes in trun on motor admin strategy');
                        resolve(true);
                    } else {
                        console.log('error in trun on motor admin strategy');
                        reject(false);
                    }
